...
class binarySearchTree : public bsTree<K,E>,public linkedBinaryTree<pair<const K, E> >
{
   public:
      ...
      bool empty() const {return linkedBinaryTree<pair<const K, E> >::treeSize == 0;}
      int size() const {return linkedBinaryTree<pair<const K, E> >::treeSize;}
      pair<const K, E>* find(const K& theKey) const;
      void insert(const pair<const K, E>& thePair);
      void erase(const K& theKey);

      // additional method of bsTree
      void ascend() { linkedBinaryTree<pair<const K, E> >::inOrderOutput();}
};
