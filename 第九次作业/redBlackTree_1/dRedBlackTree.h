...
public:
	...
	BinNodePosi(K, E) insert(const pair<const K, E>& e) { //将e插入红黑树
		BinNodePosi(K, E)& x = search(e.first);//留意对_hot的设置
		BinNodePosi(K, E)& x2;
		if (x) {
			x2 = new BinNode<K, E>(e, x, NULL, NULL, -1); _size++;
		}
		else {
			x2 = new BinNode<K, E>(e, _hot, NULL, NULL, -1); _size++; //创建红节点x：以_hot为父，黑高度-1
		}
		BinNodePosi(K, E) xOld = x2;
		solveDoubleRed(x2);
